...
const availableTables = ref([
  ...
  { value: 'periodos_academicos', label: 'Períodos Académicos' },
...
const exportData = async () => {
  ...
  try {
    ...
    const newOperation = {
      ...
      estado: 'completado',
      archivo_resultado: `export_${exportForm.value.table}_${Date.now()}.${exportForm.value.format}`,
    }

    operationsHistory.value.unshift(newOperation)

    // Simular descarga
    const blob = new Blob(['Datos exportados'], { type: 'text/plain' })
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = newOperation.archivo_resultado
    a.click()
