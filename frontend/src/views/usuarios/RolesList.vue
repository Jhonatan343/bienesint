        ...
        <div class="mt-3">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">
              {{ showEditModal ? 'Editar Rol' : 'Nuevo Rol' }}
            </h3>
            <button
              @click="closeModal"
          ...
          <form @submit.prevent="showEditModal ? updateRol() : createRol()">
            <div class="space-y-4">
              <div>
            ...
            <div class="flex justify-end space-x-3 mt-6">
              ...
              >
                <i :class="loading ? 'bx bx-loader-alt animate-spin' : showEditModal ? 'bx bx-save' : 'bx bx-plus'" class="text-sm mr-2"></i>
      ...
      >
        <div class="mt-3">
          ...
          <div class="mt-3 text-center">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Eliminar Rol</h3>
            <div class="mt-2 px-7 pt-6">
              <p class="text-sm text-gray-500 dark:text-gray-400">
                ¿Estás seguro de que quieres eliminar el rol "{{ rolToDelete?.nombre_rol }}"? Esta
          ...
          <div class="flex justify-end space-x-3 mt-6">
            ...
            <button
              ...
              class="px-4 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
            >
              <i :class="loading ? 'bx bx-loader-alt animate-spin' : 'bx bx-trash'" class="text-sm mr-2"></i>
...
export default {
  ...
  setup() {
    ...
    const filteredRoles = computed(() => {
      ...
      if (searchQuery.value) {
        filtered = filtered.filter(
          rol =>
            rol.nombre_rol.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            (rol.descripcion &&
              rol.descripcion.toLowerCase().includes(searchQuery.value.toLowerCase()))
        )
      }
    ...
    const stats = computed(() => {
      ...
      const permisosAsignados = roles.value.reduce(
        (acc, rol) => acc + (rol.permisos?.length || 0),
        0
      )
      const usuariosAsignados = roles.value.reduce((acc, rol) => acc + (rol.usuarios_count || 0), 0)

      return { total, permisosAsignados, usuariosAsignados }
    })
    ...
    const loadRoles = async () => {
      loading.value = true
      try {
        const response = await fetch('/api/v1/roles?include=permisos,usuarios_count')
        if (response.ok) {
    ...
    const updateRol = async () => {
      loading.value = true
      try {
        const response = await fetch(`/api/v1/roles/${form.value.id_rol}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('auth_token')}`,
    ...
    const confirmDelete = async () => {
      loading.value = true
      try {
        const response = await fetch(`/api/v1/roles/${rolToDelete.value.id_rol}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('auth_token')}`,
          },
